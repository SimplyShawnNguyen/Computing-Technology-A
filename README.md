# Vision-Only Surface Alignment Using Optical Flow Divergence (WILL CHANGE DURING THE PROGRESS OF THE PROJECT)
...
The project is based on optical flow divergence, which provides information about how close the camera is to a surface (time-to-contact). More advanced approaches, such as maximum divergence (max-div), allow control of both approach speed and direction for docking tasks.
...
3. Compute optical flow between frames
4. Extract motion vectors (fx, fy)
5. Visualise motion using arrows or other methods
6. Repeat continuously for real-time output
...
* Object movement is easier to detect than forward or backward camera motion
* Optical flow direction is opposite to camera movement
* Flat (low-texture) surfaces produce weak and noisy results
* Faster movement produces slightly clearer motion signals
...
* Forward and backward motion is harder to detect
...
* Detect the max-div point in the flow field
...
* Optical flow method (Farneback or Lucas–Kanade)
* Visualisation type (vectors, overlay, or heatmap)

The results were compared to analyse performance and behaviour.

---

## Project Significance

This project demonstrates that vision-only systems can estimate motion and proximity using optical flow. It also highlights the limitations of such systems, especially in low-texture environments.

The work provides a foundation for more advanced approaches such as max-div, which can support robust docking and surface alignment without requiring full 3D reconstruction.
