...
# ================================
# Previous divergence frame used for temporal smoothing
previous_divergence = None
# Temporal smoothing factor
# Higher value = smoother heatmap, lower value = faster response
temporal_alpha = 0.75
# Small blur applied before Sobel to reduce noisy vector changes
...
while True:
    if not paused:
        ...
        # Convert the 1D vector lists into 2D vector fields
        u_grid = u.reshape(grid_h, grid_w).astype(np.float32)
        v_grid = v.reshape(grid_h, grid_w).astype(np.float32)

        # Smooth the vector field before calculating derivatives.
        # This reduces small tracking noise before Sobel is applied.
        u_grid_smooth = cv2.GaussianBlur(
            u_grid,
            sobel_pre_blur_kernel,
            0
        )

        v_grid_smooth = cv2.GaussianBlur(
            v_grid,
            sobel_pre_blur_kernel,
            0
        )

        # Sobel derivative calculation:
        # du_dx = change of horizontal flow in the x direction
        # dv_dy = change of vertical flow in the y direction
        #
        # The result is divided by skip because the vector field is sampled
        # every 'skip' pixels, not every single pixel.
        du_dx = cv2.Sobel(
            u_grid_smooth,
            cv2.CV_64F,
            1,
        ...
        if previous_divergence is None:
            ...
            # Safety reset if the grid size changes
            smoothed_divergence = raw_divergence.copy()

        else:
            # Exponential moving average:
            # previous result has more weight, current result still updates the heatmap
            smoothed_divergence = (
                temporal_alpha * previous_divergence
                + (1 - temporal_alpha) * raw_divergence
            )

        # Store current smoothed divergence for the next frame
        previous_divergence = smoothed_divergence.copy()

        # Use the smoothed divergence for the rest of the pipeline
        ...
        # Smooth positive divergence slightly before heatmap display
