...
# ================================
# Section 1: Divergence Improvement Settings (Thuan)
# ================================
divergence_clip_percentile = 95   # clip top/bottom 5% outlier spikes
divergence_smooth_kernel = (5, 5) # blur full divergence map before positive clipping
...
while True:
    if not paused:
        ...
        # ================================
        # Step 7.2: Improve divergence calculation
        # Section 1: Clip outlier spikes + smooth full divergence map (Thuan)
        # ================================

        # 1. Clip extreme outlier values using percentile range
        #    Prevents a few noisy pixels from dominating the whole map
        clip_max = np.percentile(np.abs(divergence), divergence_clip_percentile)
        if clip_max > 0:
            divergence = np.clip(divergence, -clip_max, clip_max)

        # 2. Gaussian blur on the full divergence map
        #    Smooths spatial noise before separating positive/negative regions
        divergence = cv2.GaussianBlur(
            divergence.astype(np.float32),
            divergence_smooth_kernel,
            0
        )
        ...
        # Section 7: Stabilized max tracking (Thuan)
        # ================================
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(
            heatmap_uint8
        )

        # Only track when divergence is strong enough to be meaningful
        # Threshold of 30/255 filters out frames with no real motion
        if max_val > 30:
            if previous_max_loc is None:
                ...
                if distance > max_jump_distance:
                    # Jump too large — search locally around last known position
                    # instead of blindly accepting the new location
                    ...
                    if local_region.size > 0:
                        ...
                        # Accept local result only if it is at least half as strong
                    ...
                    # Small jump — alpha-blend position for smooth movement
                    stable_max_loc = (
            ...
            # No strong divergence — reset tracking so it does not latch
            # onto a stale position from a previous frame
            previous_max_loc = None

        # ================================
        # Debug output
        # ================================
        print("Section 1 + Section 7 Output (Thuan)")
        ...
        print(
            "Clipped + smoothed divergence min:",
